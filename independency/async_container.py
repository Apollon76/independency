...
class AsyncContainer:  # pylint: disable=R0903
    ...
    def create_test_container(self) -> 'AsyncTestContainer':
        ...
        _update_localns(AsyncContainer, localns)
...
class AsyncTestContainer(AsyncContainer):
    def with_overridden(
        ...
        container = AsyncTestContainer(registry, localns)
        registry[AsyncContainer] = Registration(AsyncContainer, factory=lambda: container, is_singleton=True, kwargs={})
        _update_localns(AsyncContainer, localns)
        return container
